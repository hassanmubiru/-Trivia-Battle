...
export class WalletConnectService {
  ...
  /**
   * Initialize WalletConnect provider for React Native
   */
   ...
   */
  getUri(): string | null {
    return this.provider?.session?.peer?.metadata?.url || null;
  }
  ...
  private setupEventListeners(): void {
    ...
    this.provider.on('display_uri', (uri: string) => {
      console.log('[WalletConnect] Received display_uri:', uri);
      this.emit('uri', uri);
    });
  }
